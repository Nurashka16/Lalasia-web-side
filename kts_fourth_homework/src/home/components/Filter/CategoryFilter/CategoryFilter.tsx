...
const CategoryFilter = observer(() => {
  ...
  const foundSelectedCategories = (arr: number[]) => {
    const result: Option[] = [];
  ...
  const [selectedCategories, setSelectedCategories] = useState(
    foundSelectedCategories(filter.selectedFilterIds)
  );

  const onClickCategory = (id: number) => {
    filter.toggleCategory(id);
    setSelectedCategories(foundSelectedCategories(filter.selectedFilterIds));
  };
